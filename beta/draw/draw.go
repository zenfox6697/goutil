...
func DrawStringOnImageAndSave(imagePath string, imageData []byte, infos []*DrawTextInfo) (err error) {
	...
	//新建笔刷
	// textBrush, _ := NewTextBrush("fonts/sarasa-mono-sc-regular.ttf", 50, image.Black, 50)
	textBrush, _ := NewTextBrush("fonts/H-TTF-BuMing-B-2.ttf", 24, image.NewUniform(color.RGBA{
		R: 0,
		G: 0,
		B: 0,
		A: 255,
	...
	c.SetDst(des)
	textBrush.FontColor = image.NewUniform(color.RGBA{R: 0xFF, G: 0, B: 0, A: 255})
...
func DrawRectOnImageAndSave(imagePath string, imageData []byte, infos []*DrawRectInfo) (err error) {
	...
	des := Image2RGBA(backgroud)
	//新建笔刷
	textBrush, _ := NewTextBrush("fonts/H-TTF-BuMing-B-2.ttf", 15, image.Black, 15)
	for _, info := range infos {
		...
		c.SetClip(des.Bounds())
		c.SetDst(des)
		cGreen := image.NewUniform(color.RGBA{R: 0, G: 0xFF, B: 0, A: 255})
		c.SetSrc(cGreen)
